...
import { canonicalWorkspacePath, installAgentRuntimeWhenReady, listWorkspaceRecords, liveWorkspaceAgentCount, projectConnectionsView, readWorkspaceConfigCached, reconcileWorkspaceRestricts, withWorkspaceWrite, workspaceConnectionStatus, workspaceMountErrorsView, workspaceRestrictErrorView, workspaceToolSchemas, writeWorkspaceConfigFile } from "./workspace-runtime.js";
...
function summarizeWorkspaceRow(ctx, wsPath, server, mountError) {
	...
	// serverName 同时被其他项目占着：第二份 mcp-client 必然撞名启动失败，而 serverName 又是日志、
	// 工具名、注册表的共同键。保存路径会拦，但手工编辑 `.dsh/mcp.json` 能绕过——说清楚和谁撞了。
	if (conn.duplicateOwners.length) {
		row.duplicateOwners = conn.duplicateOwners;
		const notice = `serverName "${server.name}" 同时被以下项目使用，进程内只能跑一份：${conn.duplicateOwners.join("、")}`;
...
let McpManagerGateway = (() => {
	...
	return class McpManagerGateway extends _classSuper {
		...
		async tools(payload) {
			// 契约声明的是一个 payload 对象：全局 MCP 只需 name，项目 MCP 还要 wsPath。serverName 的全 app
			// 唯一性只在保存路径被校验（workspaceNameTaken），手工编辑 `.dsh/mcp.json` 能造出两个
			// 项目同名；光按名字扫描就会把 A 项目的工具列到 B 项目的详情里。
