...
import { canonicalWorkspacePath, installAgentRuntimeWhenReady, listWorkspaceRecords, liveWorkspaceAgentCount, readWorkspaceConfigCached, reconcileWorkspaceRestricts, withWorkspaceWrite, workspaceConnectionStatus, workspaceMountErrorsView, workspaceRestrictErrorView, writeWorkspaceConfigFile } from "./workspace-runtime.js";
...
let McpManagerGateway = (() => {
	...
	return class McpManagerGateway extends _classSuper {
		...
		async getWorkspaceView(payload) {
			...
			const mountErrorNames = new Set(workspaceMountErrorsView(wsPath).map((m) => m.serverName));
			const servers = config.servers.map((server) => {
				const row = workspaceServerView(server);
				// 共享连接下 Host 可枚举真实连接态：有工具=已连接，建连但无工具=连接中，
				// 挂载失败=failed，无会话占用=待会话。
				if (!server.disabled) {
					const conn = workspaceConnectionStatus(this.ctx, wsPath, server.name);
					if (mountErrorNames.has(server.name)) { row.status = "failed"; row.phase = "failed"; }
					else if (conn.toolCount > 0) { row.status = "connected"; row.phase = "active"; row.toolCount = conn.toolCount; }
					else if (conn.mounted) { row.status = "connecting"; row.phase = "waiting"; }
					else { row.status = "idle"; row.phase = "idle"; }
				}
				return row;
			});
...
export { checkForUpdate, McpManagerGateway, McpManagerGateway as default };