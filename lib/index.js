...
function applyWorkspaceRestrict(ctx, agentCtx, agent, exclude) {
	...
	if (!deny.length) { state.restrictKey = key; return; }
	try {
	...
	return class McpManagerGateway extends _classSuper {
		...
		async setWorkspaceExclude(payload) {
			...
			return withWorkspaceWrite(wsPath, async () => {
				...
				// 屏蔽/取消屏蔽只改项目配置，不会触发 tools/change，需主动同步到运行中的会话。
				// 实测：取消屏蔽（撤销 restrict）当场生效；新增屏蔽只有下一个会话才拿得到，
				// 宿主的 tools.restrict() 在 agent setup 之后追加不再进入该会话的工具视图。
				await reconcileWorkspaceRestricts(this.ctx, wsPath);
				const note = hidden
					? `已在项目内屏蔽 ${serverName}；运行中的会话将在下次生效`
					: `已取消屏蔽 ${serverName}`;
				return { note, exclude };
			});
		}
...
export { applyWorkspaceRestrict, composeAgentSetup, installAgentRuntime, installAgentRuntimeWhenReady, workspaceMountErrorsView };