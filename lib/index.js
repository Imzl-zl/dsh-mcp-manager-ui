...
function summarize(entry, counts, managed, conflict = false) {
	...
	const row = { serverName: live.serverName, enabled: !entry.disabled, managed: !!managed, conflict, transport, phase, toolCount: counts[live.serverName] || 0 };
...
let McpManagerGateway = (() => {
	...
	let _list_decorators, _status_decorators, _enable_decorators, _disable_decorators, _reconnect_decorators, _tools_decorators, _add_decorators, _update_decorators, _remove_decorators, _previewImport_decorators, _importJson_decorators;
	return class McpManagerGateway extends _classSuper {
		...
		async list() {
			...
			const counts = countTools(this.ctx);
			...
			for (const [name, entries] of groups) {
				...
				servers.push(summarize(entry, counts, managed, conflict));
			}
			return { servers };
		}
		async status(name) {
			...
			return { server: summarize(entry, countTools(this.ctx), !conflict && entry.options.id === profileId ? state.byName.get(value) : null, conflict) };
		}
		async enable(name) {
		...
		async tools(name) {
			...
			return { tools: this.ctx.tools.schemas().filter((schema) => schema.name.startsWith(prefix)).map((schema) => ({ name: schema.name, description: schema.description || "" })) };
		}
		async add(spec) {
