...
function installAgentRuntime(ctx) {
	...
	if (wrappedAgentsServices.has(agentsService)) return () => {};
	const restore = [];
	for (const method of ["create", "resume"]) {
		const original = agentsService[method];
		if (typeof original !== "function") continue;
		agentsService[method] = function (...args) {
	...
	const disposeToolsChange = ctx.on?.("tools/change", async () => {
	...
	wrappedAgentsServices.add(agentsService);
	const cleanup = () => {
		if (!wrappedAgentsServices.has(agentsService)) return;
		wrappedAgentsServices.delete(agentsService);
	...
	return class McpManagerGateway extends _classSuper {
		...
		async getWorkspaceView(payload) {
			...
			return {
				...
				error: config.error || "",
				mountErrors,
				revision: revisionOf({ servers, exclude: config.exclude, global: globalResult.revision, mountErrors }),
				servers,
...
export { applyWorkspaceRestrict, composeAgentSetup, installAgentRuntime, workspaceMountErrorsView };