...
// setup 是插件参与 agent 生命周期的官方入口；本函数是这条链的**中间层**：
// 自己用 agent 做事，再原样把 (agentCtx, agent) 传给下一层。
// agent 只能走官方途径拿——dsh-agent-loop 把它作为 setup 的第二个参数传入
// （`setup?.(prepared.agent.ctx, prepared.agent)`）。ctx 是 agent 的 scope context，
// 上面没有 agent 属性；从 ctx 上读它会被 cordis 服务守卫拒绝
// （cannot get property "agent" without inject），而 setup 抛错会让会话的创建与恢复直接失败。
// （早年替身恰好把 agent 挂在 ctx 上，所以这个缺陷被 40+ 个用例掩盖过。）
function composeAgentSetup(ctx, callerSetup, generation) {
	return async (agentCtx, agent) => {
		if (!agent) {
			// 官方契约保证第二个参数就是 agent（就取在同一个调用点上）。拿不到只能是契约变了：
			// 明确告警后放行会话，不假装挂上了，也不拿自己的猜测去填一个 agent。
			ctx.logger?.warn?.("mcp-manager: setup 未收到 agent 参数（dsh-agent-loop 契约已变？），本次不挂载项目 MCP");
		...
		// 本函数是 setup 链的中间层：必须把官方契约 (agentCtx, agent) 原样透传给下一层。
		// 宿主的 setup 会读 agent.session（例如 subagent 要 append 回描述事件）；少传一个
		// 参数就是 `Cannot read properties of undefined (reading 'session')`，而 setup
		// 抛错会让整个会话的创建与恢复失败。
		return (await callerSetup?.(agentCtx, agent)) ?? undefined;
	};
}
