...
const ServerDetail = ({ server, tools, onToggle, onReconnect, onEdit, onRemove, busy, onReveal, revealed, onCopy, excluded, onToggleExclude }) => {
  ...
  return h('div', { className: 'dsh-mcp-detail' },
    h('div', { className: 'dsh-mcp-d-head' },
      h('div', null,
        ...
        h('div', { className: 'dsh-mcp-d-meta' },
          ...
          isWorkspace && server.duplicateOwners?.length ? h(Badge, { cls: 's-err' }, 'serverName 被占用') : null,
        ),
      ),
    ...
    isWorkspace && server.duplicateOwners?.length ? h('div', { className: 'dsh-mcp-log warn' }, 'serverName 在整个进程内只能跑一份，但以下项目也定义了同名服务器（面板保存会拦，手工编辑 .dsh/mcp.json 能绕过）：' + server.duplicateOwners.join('、') + '。请给其中一个换名。') : null,
...
const McpTab = (props) => {
  ...
  const inner = h(
    ...
    scope !== 'global' && mountErrors.length ? h('div', { className: 'dsh-mcp-log' }, mountErrors.length + ' 个项目 MCP 未能挂载到会话：' + mountErrors.map((m) => m.serverName).join('、') + '。点开对应条目查看具体错误。'
      // serverName 在应用根全局唯一，同一项目的第二个会话必然挂不上：并发会话数是这类
      // 失败最常见的原因，直接摆出来，省去用户逐条点开才发现是冲突。
      + (liveAgents > 1 ? '　本项目当前有 ' + liveAgents + ' 个会话在运行，serverName 只能被其中一个占用。' : '')) : null,
