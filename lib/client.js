...
const ServerDetail = ({ server, tools, onToggle, onReconnect, onEdit, onRemove, busy, onReveal, revealed }) => {
  if (!server)
  ...
  const RevealBtn = ({ field, keyName, label, fallback }) => {
    ...
    return h('span', { className: 'dsh-mcp-secret' },
      ...
      h('button', { type: 'button', className: 'dsh-mcp-secret-btn', disabled: state?.pending, title: state?.value !== undefined ? '隐藏 ' + label : '显示 ' + label, 'aria-label': '显示/隐藏 ' + label, onClick: () => onReveal(server, field, keyName) }, state?.pending ? '…' : h(Icon, { d: ICONS[state?.value !== undefined ? 'eyeOff' : 'eye'] })),
    );
  };
...
const AddEditModal = ({ open, onClose, onSubmit, busy, initial, onRevealValue }) => {
...
const McpTab = (props) => {
  ...
  const inner = h(
    ...
    h(AddEditModal, { open: addOpen, onClose: () => { setAddOpen(false); setEditTarget(null); }, onSubmit, busy, initial: editTarget, onRevealValue: revealValue }),
