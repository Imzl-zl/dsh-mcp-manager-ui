  ...
  return h('div', { className: 'dsh-mcp-detail' },
    ...
    h('div', { className: 'dsh-mcp-card' },
      ...
      h('div', { className: 'dsh-mcp-tools' },
        tools.length === 0
          ? h('div', { className: 'dsh-mcp-empty-list' }, server.toolCountAmbiguous ? '工具归属存在命名歧义' : '暂无工具')
...
const AddEditModal = ({ open, onClose, onSubmit, busy, initial, onRevealValue, revealEpoch }) => {
...
const BuiltinInstallModal = ({ open, onClose, call, busy, setBusy, onInstalled }) => {
  const dialogRef = useRef(null);
  ...
  const install = async () => {
    ...
    try {
      const res = await call('installBuiltins', { ids: selected });
...
const McpTab = (props) => {
  ...
  const load = useCallback(async (notify = false) => {
    ...
    try {
      ...
      if (res && res.ok) {
        ...
        // 兼容旧 Host（未返回 revision 字段）：直接 setServers；
        // 新 Host 才用指纹比较跳过无变化的重渲染。
  ...
  const onToggle = (s) => {
    ...
    // 清空 revision 指纹，强制下一次 load 无条件 setServers：
    // 成功时同步真实状态，失败时回滚乐观值（否则 revision 不变会跳过回滚）。
  ...
  const onRemove = (s) => {
    if (confirm('确认移除 MCP「' + s.serverName + '」？将从当前 Web profile 配置中删除。')) {
      clearRevealed();
      act('removeServer', s.serverName, '已移除 ' + s.serverName);
    }
  };
  ...
  const revealValue = useCallback(async (server, field, key) => {
    ...
    const res = await call('reveal', { name: server.serverName, field, key });
  ...
  const inner = h(
    ...
    h(
      ...
      h(
        ...
        h(Btn, { icon: 'plug', onClick: () => setBuiltinOpen(true), children: '内置 MCP' }),
    ...
    h(BuiltinInstallModal, { open: builtinOpen, onClose: () => setBuiltinOpen(false), call, busy, setBusy, onInstalled: onBuiltinsInstalled }),
    h(AddEditModal, { open: addOpen, onClose: () => { setAddOpen(false); setEditTarget(null); }, onSubmit, busy, initial: editTarget, onRevealValue: revealValue, revealEpoch: revealGeneration.current }),
