...
.dsh-mcp-tabs{display:flex;gap:6px;align-items:center;overflow-x:auto;scrollbar-width:none;position:relative;flex:none;padding:2px 0}.dsh-mcp-tabs::-webkit-scrollbar{display:none}.dsh-mcp-tab{display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border-radius:9px;border:1px solid var(--mcp-line);background:var(--mcp-bg-1);color:var(--mcp-mut);font-size:12px;cursor:pointer;flex:none;white-space:nowrap;transition:background .13s,border-color .13s,color .13s;line-height:1.4}.dsh-mcp-tab:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l3)}.dsh-mcp-tab.sel{color:var(--dsw-alias-label-primary);border-color:var(--mcp-accent);background:var(--dsw-alias-interactive-bg-hover-accent);font-weight:600}.dsh-mcp-tab-count{font-size:10px;font-weight:500;padding:0 6px;border-radius:999px;background:var(--mcp-line);color:var(--mcp-mut);line-height:15px}.dsh-mcp-tab-pop{position:absolute;top:calc(100% + 5px);right:0;z-index:60;background:var(--mcp-bg-2);border:1px solid var(--mcp-line);border-radius:11px;box-shadow:0 14px 34px rgba(0,0,0,.28);padding:6px;display:flex;flex-direction:column;gap:2px;min-width:170px}.dsh-mcp-tab-pop .dsh-mcp-tab{width:100%}.dsh-mcp-ws-body{width:288px;min-width:248px;display:flex;flex-direction:column;gap:8px;border:1px solid var(--mcp-line);border-radius:12px;background:var(--mcp-bg-2);padding:10px;overflow:hidden}.dsh-mcp-section{display:flex;flex-direction:column;gap:6px;min-height:0}.dsh-mcp-section+.dsh-mcp-section{border-top:1px solid var(--mcp-line);padding-top:8px}.dsh-mcp-section-title{font-size:10.5px;font-weight:650;color:var(--mcp-mut);text-transform:uppercase;letter-spacing:.6px;display:flex;align-items:center;gap:6px;flex:none}.dsh-mcp-section-title::after{content:"";flex:1;height:1px;background:var(--mcp-line)}.dsh-mcp-global-list{display:flex;flex-direction:column;gap:3px;overflow-y:auto;min-height:0;margin:0 -4px;padding:0 4px}
.dsh-mcp-hide-toggle{display:inline-flex;align-items:center;align-self:center;gap:4px;flex:none;margin-left:auto;padding:4px 8px;border:1px solid var(--mcp-line);border-radius:8px;background:var(--mcp-bg-1);color:var(--mcp-mut);font-size:11px;cursor:pointer;white-space:nowrap;transition:border-color .13s,color .13s}
.dsh-mcp-hide-toggle:hover{border-color:var(--mcp-accent);color:var(--dsw-alias-label-primary)}
.dsh-mcp-hide-toggle input{width:14px;height:14px;margin:0;accent-color:var(--mcp-accent);cursor:pointer}
...
const ServerList = ({ servers, selected, query, onQuery, onSelect, onToggle, busy, transportFilter, statusFilter, onTransportFilter, onStatusFilter }) =>
  h('div', { className: 'dsh-mcp-side' },
    ...
    h('div', { className: 'dsh-mcp-filters' },
...
// 项目 tab 中的「全局 MCP（可屏蔽）」列表：只读 + 隐藏开关，全局数量多时可搜索过滤。
const GlobalServerList = ({ servers, exclude, onToggleExclude, busy }) => {
  const [q, setQ] = useState('');
  const hidden = new Set(exclude || []);
  const query = (q || '').toLowerCase();
  const list = query ? servers.filter((s) => s.serverName.toLowerCase().includes(query)) : servers;
  return h('div', { className: 'dsh-mcp-global-list' },
    servers.length === 0
      ? h('div', { className: 'dsh-mcp-empty-list' }, '没有全局 MCP')
      : h(React2.Fragment, null,
          servers.length > 6 ? h('div', { className: 'dsh-mcp-global-search' }, h(Icon, { d: ICONS.search }), h('input', { placeholder: '搜索全局 MCP…', value: q, onChange: (e) => setQ(e.target.value) })) : null,
          list.length === 0
            ? h('div', { className: 'dsh-mcp-empty-list' }, '没有匹配的全局 MCP')
            : list.map((s) =>
                h('div', { key: s.serverName, className: 'dsh-mcp-item' },
                  h(Dot, { phase: s.status || s.phase }),
                  h('div', { className: 'dsh-mcp-item-main' },
                    h('div', { className: 'dsh-mcp-item-name' }, s.serverName),
                    h('div', { className: 'dsh-mcp-item-sub' },
                  ...
                  h('label', { className: 'dsh-mcp-hide-toggle', title: hidden.has(s.serverName) ? '取消屏蔽（此项目可重新看到）' : '屏蔽（此项目的会话不再看到此全局 MCP）' },
                    h('input', { type: 'checkbox', checked: hidden.has(s.serverName), disabled: busy, onChange: () => onToggleExclude(s.serverName, !hidden.has(s.serverName)) }),
                    h('span', null, hidden.has(s.serverName) ? '已屏蔽' : '屏蔽'),
                  ),
                ),
              ),
        ),
...
const ServerDetail = ({ server, tools, onToggle, onReconnect, onEdit, onRemove, busy, onReveal, revealed, onCopy }) => {
  if (!server)
  ...
  const status = server.status || server.phase || 'waiting';
  const statusCls = server.enabled ? (status === 'connected' ? 's-on' : status === 'failed' ? 's-err' : '') : 's-off';
  const statusText = server.enabled ? (status === 'connected' ? '已连接' : status === 'failed' ? '连接失败' : status === 'unknown' ? '状态未知' : status === 'loading' || status === 'waiting' ? '连接中…' : '已停止') : '已禁用';
  const mut = (v) => {
  ...
  return h('div', { className: 'dsh-mcp-detail' },
    h('div', { className: 'dsh-mcp-d-head' },
      h('div', null,
        ...
        h('div', { className: 'dsh-mcp-d-meta' },
          ...
          h(Badge, null, server.scope === 'workspace' ? '随会话挂载' : server.toolCount + ' 个工具'),
        ),
      ),
      h('div', { className: 'dsh-mcp-d-actions' },
        ...
        h(Btn, { icon: 'refresh', onClick: () => onReconnect(server), disabled: busy || !server.enabled || !server.managed || server.scope === 'workspace', title: server.scope === 'workspace' ? '项目 MCP 随会话自动挂载，无需手动重连' : '重连', children: '重连' }),
    ...
    h('div', { className: 'dsh-mcp-card' },
      ...
      h('div', { className: 'dsh-mcp-kv' },
        ...
        h('b', null, '配置来源'), h('span', null, server.conflict ? '存在重复 serverName（冲突，只读）' : server.scope === 'workspace' ? '项目配置 .dsh/mcp.json（可编辑，随会话挂载）' : server.managed ? '当前 Web profile（可编辑）' : '其他 bundle / Agent（只读）'),
...
const McpTab = (props) => {
  ...
  useEffect(() => {
    load(false);
    const dispose = timer.interval(() => load(false), 5000);
    return dispose;
  }, [load, timer]);
  ...
  const inner = h(
    ...
    error ? h('div', { className: 'dsh-mcp-d-err' }, error) : null,
    scope !== 'global' && mountErrors.length ? h('div', { className: 'dsh-mcp-d-err' }, '以下项目 MCP 未能挂载（可能已被其他会话占用或连接失败）：' + mountErrors.map((m) => m.serverName + '（' + m.error + '）').join('；')) : null,
    scope === 'global'
      ? h(
          'div',
          { className: 'dsh-mcp-body' },
          h(ServerList, { servers: filtered, selected, query, onQuery: setQuery, onSelect: setSelected, onToggle, busy, transportFilter, statusFilter, onTransportFilter: setTransportFilter, onStatusFilter: setStatusFilter }),
          h(ServerDetail, {
      ...
      : h(
          ...
          h('div', { className: 'dsh-mcp-ws-body', 'data-ws-path': scope },
            h('div', { className: 'dsh-mcp-section' },
              h('div', { className: 'dsh-mcp-section-title' }, '此项目的 MCP（' + servers.length + '）'),
              h(ServerList, { servers: filtered, selected, query, onQuery: setQuery, onSelect: setSelected, onToggle, busy, transportFilter, statusFilter, onTransportFilter: setTransportFilter, onStatusFilter: setStatusFilter }),
            ),
            h('div', { className: 'dsh-mcp-section' },
              h('div', { className: 'dsh-mcp-section-title' }, '全局 MCP（此项目可见，可屏蔽）'),
              h(GlobalServerList, { servers: globalServers, exclude, onToggleExclude, busy }),
            ),
          ),
          h(ServerDetail, {
            server: selectedServer,
