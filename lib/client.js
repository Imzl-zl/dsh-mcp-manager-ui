...
const ServerList = ({ servers, total, selected, query, onQuery, onSelect, onToggle, busy, transportFilter, statusFilter, onTransportFilter, onStatusFilter, embedded, workspace, emptyHint, mountErrors }) => {
  ...
  // 项目 MCP 现在是共享长驻连接，Host 侧能拿到真实连接态：有工具=已连接，建连中=connecting，
  // 挂载失败=failed，尚无会话占用=idle（待会话）。状态点与全局一致。
  const dotPhase = (s) => (mountError(s) ? 'failed' : s.status || s.phase);
  // 项目行的工具/状态副文案。
  const wsSub = (s) => {
    if (mountError(s)) return '挂载失败';
...
const ServerDetail = ({ server, tools, onToggle, onReconnect, onEdit, onRemove, busy, onReveal, revealed, onCopy, excluded, onToggleExclude }) => {
  ...
  const status = mountFailed ? 'failed' : server.status || server.phase || 'waiting';
  ...
  const statusText = !server.enabled
    ...
    : mountFailed
      ? '挂载失败'
      : isWorkspace
        ? (status === 'connected' ? '已连接（本项目会话共享）' : status === 'failed' ? '连接失败' : status === 'connecting' ? '连接中…' : '待会话挂载')
  ...
  return h('div', { className: 'dsh-mcp-detail' },
    ...
    isWorkspace && server.configStale ? h('div', { className: 'dsh-mcp-log warn' }, '配置已修改，但本项目的共享连接是按修改前的配置建立的，仍在运行。新开的会话也会复用它；等本项目所有会话都结束后，下一个会话才会用新配置建连。') : null,
