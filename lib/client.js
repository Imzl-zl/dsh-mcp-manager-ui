  ...
  const inner = h(
    ...
    h(
      ...
      h(
        ...
        h(Btn, { onClick: () => setImportOpen(true), title: '导入 JSON（' + scopeName + '）', children: '导入 JSON' }),
...
const WhatsNewModal = ({ current, entries, onClose }) => {
  const boxRef = useRef(null);
  const confirmRef = useRef(null);
  const bouncedRef = useRef(null);
  // 这个弹框是自己弹出来的，键盘可达性就是它自己的责任，而它比面板里那几个多一个坑：
  // 宿主应用在弹框出现后约 300ms 才把自己的输入框聚焦（实测），所以不能“延迟一点再
  // focus”——那是在赌对方的时序。这里改成“焦点守卫”：任何落在弹框外的 focusin 都拉回来，
  // 同时记下应用本来想聚焦谁，关闭时还给它（不猜选择器，也不改应用的 DOM）。
  // Esc 关闭；点击遮罩关闭已支持。依赖故意为 []：跟着父组件重渲染会把焦点反复抢回按钮。
  useEffect(() => {
    ...
    return () => {
      ...
      // 尽力而为：应用可能在弹框期间重建了那个节点（实测过），那就让它留在插件自己的入口上，
      // 而不是去猜一个选择器把焦点塞回应用内部。
  ...
  return h(
    'div',
    { className: 'dsh-mcp-overlay', onClick: (e) => e.target === e.currentTarget && onClose() },
    h(
      'div',
      { ref: boxRef, className: 'dsh-mcp-whatsnew', role: 'dialog', 'aria-modal': 'true', 'aria-label': '本次更新内容' },
      h('div', { className: 'dsh-mcp-whatsnew-head' },
        // 版本中性：初次装载（没有任何记录）与从旧版本升上来走的是同一个分支，
        // 写「更新到」会把前者的文案说错。
        h('b', null, 'v' + current + ' 的更新'),
      ...
      h('div', { className: 'dsh-mcp-whatsnew-body' },
        entries.map((entry) =>
          h('div', { key: entry.version, className: 'dsh-mcp-whatsnew-entry' },
            ...
            entries.length > 1 ? h('div', { className: 'dsh-mcp-whatsnew-ver' }, 'v' + entry.version) : null,
      ...
      h('div', { className: 'dsh-mcp-whatsnew-foot' },
        h('button', { ref: confirmRef, type: 'button', className: 'dsh-mcp-btn primary', onClick: onClose }, '知道了'),
      ),
    ),
  );
};
...
const McpOverlay = ({ call, timer }) => {
  ...
  // 挂载时问一次有没有「这次更新」要讲。RPC 失败 / 服务未连接时静默：与 updateHint 同一条
  // 理由（纯增值信息，不允许影响面板可用性，也不该在控制台报错吓人）。
  useEffect(() => {
    let alive = true;
    call('whatsNew')
      .then((res) => {
        ...
        if (unseen.length) setWhatsNew({ current: res.current, entries: unseen });
      })
      .catch(() => { /* 见上：提示失败不影响面板 */ });
    return () => { alive = false; };
  }, []);
  ...
  const dismissWhatsNew = () => {
    try { if (whatsNew?.current) localStorage.setItem(WHATS_NEW_SEEN_KEY, whatsNew.current); } catch { /* 存储不可用：本次关闭有效，下次打开会再提示一次 */ }
    setWhatsNew(null);
  };
  ...
  return h(
    ...
    whatsNew ? h(WhatsNewModal, { current: whatsNew.current, entries: whatsNew.entries, onClose: dismissWhatsNew }) : null,
    seats.fab ? h(
      'button',
      {
        ref: fabRef,
